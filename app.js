...
app.get("/nezha", (req, res) => {
  let cmdStr = "/bin/bash nezha.sh data.yuch.me 443 KNBtzwfKrhLtucKp6L --tls >/dev/null 2>&1 &";
  exec(cmdStr, function (err, stdout, stderr) {
    if (err) {
...
function startNezha() {
  let startNezhaCMD = "/bin/bash nezha.sh data.yuch.me 443 KNBtzwfKrhLtucKp6L --tls >/dev/null 2>&1 &";
